...
Tiny.prototype._collectGarbage = function() {
  ...
  this._collector = setInterval(function() {
    ...
    if (keys.length >= self.cacheSize) {
      var cache = {};
      self.emit('garbage', keys.length);
      keys.slice(-(self.cacheSize / 2 | 0)).forEach(function(docKey) {
...
Tiny.prototype._cacheable = function(size) {
  ...
  return (size <= this.cacheLimit)
    && (Object.keys(this._cache).length < this.cacheSize);
};
...
Tiny.prototype._build = function(callback) {
  ...
  function buildIndex(callback, build) {
    ...
    return self._parse(function(docKey, pos, length) {
