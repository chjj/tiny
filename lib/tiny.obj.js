...
Tiny.prototype._build = function(callback) {
  ...
  function buildIndex(callback, build, data) {
    ...
    */
    ...
    return self._parse(self._total, function(docKey, pos, length) {
      //var previous = self._index[docKey];
      self._index[docKey] = [pos, length];
      //self._index[docKey].previous = previous;
    }, function(err, total) {
      if (err) return callback(err);
  ...
  function buildCache(callback) {
    ...
    return parallel(self._index, function(val, next, docKey) {
      return self._ensureIntegrity(docKey, function(err, slot) {
        if (err) return next();

        if (self._cacheable(slot[1])) {
          return self._lookup(slot, function(err, data) {
            if (err) return next();
            self._cache.set(docKey, data);
            return next();
          });
        }
...
Tiny.prototype._ensureIntegritySync = function(docKey, callback) {
  var self = this
    , fd = this._fd
    , slot = this._index[docKey]
    , previous = slot.previous
