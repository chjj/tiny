...
Tiny.prototype._collectGarbage = function() {
  ...
  this._collector = setInterval(function() {
    var docKeys = Object.keys(self._cache);
    if (docKeys.length >= self.cacheNumber) {
      var cache = {};
      docKeys.slice(-(self.cacheNumber / 2 | 0)).forEach(function(docKey) {
...
Tiny.prototype._cacheable = function(size) {
  return Object.keys(this._cache).length < this.cacheNumber;
};
...
Tiny.prototype._build = function(callback) {
  ...
  return this._parse(function(key, pos, length) {
    var parts = key.split(self.tokens.delimeter);
    var docKey = parts[0];
    var propKey = parts[1];
    if (!index[docKey]) {
      index[docKey] = {};
    }
    var previous = index[docKey][propKey];
    index[docKey][propKey] = [pos, length];
    index[docKey][propKey].previous = previous;
  }, function(err, total) {
    if (err) return callback(err);

    return parallel(index, function(val, next, docKey) {
      return parallel(index[docKey], function(val, next, propKey) {
        return self._ensureIntegrity(index, docKey, propKey, function(err, slot) {
          if (err) return next();
...
Tiny.prototype._ensureIntegrity = function(index, docKey, propKey, callback) {
  var self = this
    , fd = this._fd
    , slot = index[docKey][propKey]
  ...
  function recheck() {
    ...
    index[docKey][propKey] = previous;
    return self._ensureIntegrity(index, docKey, propKey, callback);
  }
  ...
  return fs.read(fd, ch, 0, 1, pos, function(err, bytes) {
    ...
    return fs.read(fd, ch, 0, 1, pos + length - 1, function(err, bytes) {
      ...
      return check
        ? callback(null, index[docKey][propKey])
        : recheck();
    });
  });
};
...
Tiny.prototype._set = function(docKey, data, callback, action) {
  ...
  if (action !== 'update' && index[docKey]) {
    if (!cache[docKey]) {
      return this.get(docKey, function(err, data) {
        if (err) return callback(err);
        cache[docKey] = {};
        Object.keys(data).forEach(function(propKey) {
          cache[docKey][propKey] = data[propKey];
        });
        return self._set(docKey, data, callback, action);
      });
    }
...
Tiny.prototype.commit = function(callback) {
  ...
  queue = queue.map(function(item) {
    ...
    Object.keys(item.doc).forEach(function(propKey) {
      ...
      slots[propKey][0] = self._total + total + (realKeySize - 1);
      slots[propKey][1] = propSize - realKeySize;
...
Tiny.prototype.get = function(docKey, callback, shallow) {
  ...
  var slots = this._index[docKey];
  if (!slots) {
    return callback(new Error('Not found.'));
  }

  if (!cached) {
    return parallel(slots, function(slot, next, propKey) {
      return self._lookup(slot, function(err, data) {
        if (err) return next();
        doc[propKey] = data;
        return next();
