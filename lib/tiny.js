...
Tiny.prototype._build = function(callback) {
  ...
  function buildIndex(callback, build) {
      ...
      return fs.stat(idx, function(err, stat) {
        ...
        return fs.readFile(idx, function(err, buff) {
          ...
          return fs.stat(self.name, function(err, stat) {
            ...
            if (stat.size > total) {
              Tiny.debug('Index is too small. Rebuilding...');
              return buildIndex(callback, true);
            }

            return callback(null, index, total);
          });
        });
      });
    }
    ...
    return self._parse(function(realKey, pos, length) {
      var previous = index[realKey];
      index[realKey] = [pos, length];
      index[realKey].previous = previous;
    }, function(err, total) {
      if (err) return callback(err);
      ...
      Tiny.debug('Writing index...');
      return fs.writeFile(idx, JSON.stringify([total, index]), function() {
        return callback(null, index, total);
      });
    });
  }

  function buildCache(index, callback) {
    var cache = {};
    return parallel(index, function(val, next, realKey) {
      return self._ensureIntegrity(index, realKey, function(err, slot) {
        if (err) return next();
  ...
  return buildIndex(function(err, index, total) {
    if (err) return callback(err);
    return buildCache(index, function(err, cache) {
      if (err) return callback(err);
      Object.keys(index).forEach(function(key) {
        delete index[key].previous;
      });
      self._index = index;
      self._cache = cache;
      self._total = total;
