...
Tiny.prototype._build_old = function(func) {
  var self = this
    , index = {}
    , cache = {};
      ...
      self._lookup(slot, function on(err, data) {
        if (err) {
          if (err instanceof SyntaxError) {
            debug('Corrupt record found: %s.', 
                  'Using previous value.', key);
            if (slot.previous) {
              slot = slot.previous;
              index[key] = slot;
...
Tiny.prototype._build = function(func) {
  ...
  self._parse(function(key, pos, length) {
    ...
    para(index, function(loop, slot, key) {
      self._ensureIntegrity(index, key, function(done) {
        if (err) return loop();
        ...
        switch (state) {
          case 'key':
            switch (data[i]) {
              case 0x09: 
                ...
                break;
              case 0x0A: // corrupt data, handle gracefully
                kstart = i;
                break;
            }
            break;
          case 'data':
            switch (data[i]) {
              case 0x0A:
                state = 'key';
                func(key, dstart, pos - dstart);
                key = '';
                kstart = i + 1;
                break;
              case 0x09: // corrupt data, handle gracefully
                dstart = pos + 1;
                break;
            }
            break;
